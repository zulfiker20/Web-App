...
@section('title', $meta_title ?? 'Web App - Home')
@section('content')
    <main>
                ...
                <div class="row align-items-center justify-content-center">
                    <div class="col-lg-6 mb-5 mb-lg-0">
                        <div class="block text-center text-lg-start pe-0 pe-xl-5">
                            <h1 class="text-capitalize mb-4">{{ optional($banners->first())->title ?? '' }}
                            </h1>
                            <p class="mb-4">{{ optional($banners->first())->subtitle ?? '' }}</p> <a
                                type="button" class="btn btn-primary" href="{{ route('services') }}">See
                                More<span style="font-size: 14px;" class="ms-2 fas fa-arrow-right"></span></a>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="ps-lg-5 text-center">
                            @if ($banners->isNotEmpty() && $banners->first()->img)
                                <img loading="lazy" decoding="async" src="{{ asset('uploads/' . $banners->first()->img) }}"
                                    alt="{{ $banners->first()->title ?? 'About image' }}" class="w-100">
                            @else
                                <img loading="lazy" decoding="async" src="{{ asset('uploads/default.png') }}"
                        ...
                        <div class="col-lg-4 col-md-6 service-item">
                            <a class="text-black" href="/services/{{ $service->id }}">
                                <div class="block"> <span class="colored-box text-center h3 mb-4">
                                        {{ $loop->iteration < 10 ? '0' . $loop->iteration : $loop->iteration }}</span>
                                    <h3 class="mb-3 service-title">{{ $service->title }}</h3>
                                    <p class="mb-0 service-description">{{ $service->sub_title }}</p>
                                </div>
                            </a>
                        </div>
                    @endforeach
