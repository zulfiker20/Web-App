...
@section('title', $meta_title ?? 'Blog - Web App')
@section('content')
    <main>
                ...
                <div class="row">
                                    ...
                                    <div class="col-md-6" data-aos="fade">
                                        <article class="blog-post">
                                            <div class="post-slider slider-sm rounded">
                                                <img loading="lazy" decoding="async" src="{{ asset($article->image) }}"
                                                    alt="Post Thumbnail">

                                            </div>
                                            <div class="pt-4">
                                                <p class="mb-3">{{ $article->created_at->format('M d, Y') }}</p>
                                                <h2 class="h4"><a class="text-black"
                                                        href="{{ route('blogs.show', $article->id) }}">{{ $article->title }}</a>
                                                </h2>
                                            </div>
                                        </article>
                                    </div>
                    ...
                    <div class="col-lg-3">
                        ...
                        <div class="widget widget-categories">
                            ...
                            <ul class="list-unstyled widget-list">
                                @foreach ($categories as $category)
                                    <li><a href="{{ route('blogs.category', $category->id) }}">{{ $category->title }} <small
                                                class="ml-auto">({{ $category->articles->count() }})</small></a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                        ...
                        <div class="widget">
                            ...
                            <!-- post-item -->
                            <ul class="list-unstyled widget-list">
                                @foreach ($latestArrticles as $latestArrticle)
                                    <li class="d-flex widget-post align-items-center mb-3">
                                        <a class="text-black" href="{{ route('blogs.show', $latestArrticle->id) }}">
                                            <div class="widget-post-image flex-shrink-0 me-3">
                                                <img class="rounded" loading="lazy" decoding="async"
                                                    src="{{ asset($latestArrticle->image) }}" alt="Post Thumbnail">
                                            </div>
                                        </a>
                                        <div class="flex-grow-1">
                                            <h5 class="h6 mb-0"><a class="text-black"
                                                    href="{{ route('blogs.show', $latestArrticle->id) }}">{{ $latestArrticle->title }}</a>
                                            </h5>
                                            <small>{{ $latestArrticle->created_at->format('M d, Y') }}</small>
                                        </div>
                                    </li>
                                @endforeach
                            </ul>
